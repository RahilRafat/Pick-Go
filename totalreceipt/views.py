...
from totalreceipt.serializer import RecieptSerializer, ListHistorySerializer
...
class RecieptViewSet(viewsets.ModelViewSet):
    ...
    def create(self , request,*args,**kwargs):
        ...
        subreciept_queryset=Supreceipt.objects.filter(total_reciet_fk=receipt_instance)
       
      
        total_amount = 0
        for subreciept in subreciept_queryset:
            totalsupprice = subreciept.totalsupprice or 0  
            total_amount += totalsupprice

        
        receipt_instance.total_receipt = total_amount
        ...
        'restaurant_name': receipt_instance.res_fk.name if receipt_instance.res_fk else 'Unknown',
        # 'date': receipt_instance.date.strftime("%Y-%m-%d %H:%M:%S")
    }
        print(restaurant)
...
class UserOrderHistoryView(generics.ListAPIView):
    ...
    def list(self, request):
        user = request.user  
        order_history = Supreceipt.objects.filter(total_reciet_fk__user_fk=user)  
        serializer = ListHistorySerializer(order_history, many=True)
